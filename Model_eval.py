...
# Paths
item_num = str(4)
db_request_path = '/home/aviadmin/chernousov/shared/DB-selection'
texts_path = '/home/aviadmin/chernousov/shared/models/' + item_num + '/preprocessed'
model_save_path = '/home/aviadmin/chernousov/shared/models/' + item_num + '/model.doc2vec'
model = gensim.models.Doc2Vec.load(model_save_path)


class Model(object):

    def __init__(self, name, dm, size, window,  min_count, sample, epochs,
    ...
    def evaluate_model(self):
        ...
        self.percentage = 100 - counter / len(keys) * 100
        print('Mistakes:', counter)
        print('Matching percent: ', self.percentage)

    def save_report(self):
        with open(self.report_path, 'rb+') as fp:
            reports = pickle.load(fp)
            new_report = np.array([self.name, round(self.percentage, 3), self.dm, self.size, self.window,
            ...
            new_report = pd.DataFrame(data=new_report, columns=['name', 'Match %', 'dm', 'vector size', 'window',
        ...
        pickle.dump(self.report, open(self.report_path, "wb+"))
...
for x in range(10):
    ...
    one = Model(name_init[:-7],
                random.randint(0, 1),  # dm
                random.randint(300, 500),  # size
                random.randint(3, 10),  # window
                random.randint(0, 5),  # min_count
                1e-5,  # threshold for configuring higher-frequency words downsampling
                # default is 0 (off), useful value is 1e-5.
                random.randint(3, 15),  # epochs
                random.randint(0, 1),  # hs
                random.randint(5, 40),  # negative
                0,  # dm_mean
                1,  # dm_concat
                random.randint(0, 1),  # DBOW-word
                # if set to 1 trains word - vectors( in skip - gram fashion) simultaneous
                # with DBOW doc-vector training; default is 0 (faster training of doc-vectors only).
                random.randint(1, 100)/1000,  # alpha
                random.randint(1, 10000))  # seed
    one.train_model()
    one.evaluate_model()
    one.save_report()
    one.show_report()
